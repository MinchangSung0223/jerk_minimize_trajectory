...
a_max= [40,40,40,40,40,40]';
a_min= -[40,40,40,40,40,40]';
j_min= -[40,40,40,40,40,40]';
j_max= [40,40,40,40,40,40]';
eps = [0.01;0.01;0.01;0.00;0.00;0.00];
method =5;
eomg = 0.01;
ev = 0.001;
%% ROBOT PARAMETER
...
a_traj = JointAccTrajectory(q0,qT,H*t_step,H+1,method);
j_traj = JointJerkTrajectory(q0,qT,H*t_step,H+1,method);

G_traj = ScrewTrajectory(Gpick, Gplace, H*t_step, H+1, 3);
...
%% qp Solve
A = [A_dynamic;A_min_max;A_start_constraint;A_end_constraint;A_traj_constraint];
lb= [b_dynamic;b_min;b_start_constraint;b_end_constraint;lb_traj_constraint];
ub= [b_dynamic;b_max;b_start_constraint;b_end_constraint;ub_traj_constraint];
...
FKlist2 = []
  for i = 1:1:length(q2_list)
      ...
      FKlist = [FKlist;[0,0,0,T(1,4),T(2,4),T(3,4)]];
      FKlist2 = [FKlist2;[0,0,0,G(1,4),G(2,4),G(3,4)]];
      
      %drawrobot_screw(thetalist,1,110,40,1000,0.05,0.0,false,t_step,G_traj);
  end
figure(77)
subplot(6,1,1)
plot(FKlist(:,1))
